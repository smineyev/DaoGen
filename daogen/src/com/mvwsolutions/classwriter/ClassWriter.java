...
public class ClassWriter {
	...
	public void readClass(InputStream is) throws IOException,
			CodeCheckException {
		clearClass();
	...
	CPInfo readConstant(DataInputStream classStream) throws IOException,
			CodeCheckException {
	...
	static class CPUtf8 extends CPInfo {
		...
		CPUtf8(DataInputStream classStream) throws IOException {
	...
	final int getNameAndTypeIndex(String name, String descriptor) {
		...
		for (Iterator<CPInfo> i = constantPool.iterator(); i.hasNext(); index++) {
			...
			if (constant instanceof CPNameAndType) {
				...
				if (nt.nameIndex == nameIndex
						&& nt.descriptorIndex == descriptorIndex)
					return index;
			}
		}
	...
	public final int getReferenceIndex(int tag, String className, String name,
			String descriptor) {
		...
		for (Iterator<CPInfo> i = constantPool.iterator(); i.hasNext(); index++) {
			...
			if (constant instanceof CPTypeRef) {
				...
				if (tr.classIndex == classIndex
						&& tr.nameAndTypeIndex == nameAndTypeIndex)
					return index;
			}
		}
		constantPool.add(new CPTypeRef((short) tag, classIndex,
				nameAndTypeIndex));
		return constantPool.size() - 1;
	}
	...
	public String getSourceFile() {
		SourceFileAttribute sfa = (SourceFileAttribute) attributes
				.getAttributeByType(SourceFileAttribute.typeString);
		if (sfa != null) {
	...
	public String getClassName(int classIndex) {
		return TypeParse
				.convertFromInternalClassName(getInternalClassName(classIndex));
	}
	...
	public class CPTypeRef extends CPInfo {
		...
		CPTypeRef(short t, DataInputStream classStream) throws IOException {
			super(t);
		...
		public String getSymbolName() {
			return getString(((CPNameAndType) constantPool
					.get(nameAndTypeIndex)).nameIndex);
		}

		public String getSymbolType() {
			return getString(((CPNameAndType) constantPool
					.get(nameAndTypeIndex)).descriptorIndex);
		}

		public void setSymbolType(String newType) {
			((CPNameAndType) constantPool.get(nameAndTypeIndex)).descriptorIndex = getStringIndex(newType);
		}
	...
	static class CPNameAndType extends CPInfo {
		...
		CPNameAndType(DataInputStream classStream) throws IOException {
	...
	static class CPClass extends CPInfo {
		...
		CPClass(DataInputStream classStream) throws IOException {
	...
	static class CPString extends CPInfo {
		...
		CPString(DataInputStream classStream) throws IOException {
	...
	static class CPInteger extends CPInfo {
		...
		CPInteger(DataInputStream classStream) throws IOException {
	...
	static class CPFloat extends CPInfo {
		...
		CPFloat(DataInputStream classStream) throws IOException {
	...
	static class CPLong extends CPInfo {
		...
		CPLong(DataInputStream classStream) throws IOException {
	...
	static class CPDouble extends CPInfo {
		...
		CPDouble(DataInputStream classStream) throws IOException {
