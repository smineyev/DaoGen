...
public class CodeAttribute implements Attribute {
	...
	CodeAttribute(DataInputStream classStream, ClassWriter contains)
		...
		int codeLength = classStream.readUnsignedShort() * 65536
		...
		try {
			while (ip.currentPos < code.length) {
				Instruction instr = OpCode.opCodes[NetByte
						.mU(code[ip.currentPos])].read(ip, code);
	...
	/**
	 * Replaces oldLength instructions at instruction pointer start with the
	 * instructions in newInstructions. instructionStarts for instructions after
	 ...
	 * instructions are adjusted for the difference in instruction length
	 * between the old instructions and the new instructions. Branches from
	 * outside the inserted section to within the inserted section cause a
	 * CodeCheckException. Branches within the inserted section are not
	 * adjusted; if such branches point to points after the inserted code, the
	 * branches must be set as if the code had already been inserted.
	 * 
	 * Assumes branch offsets from instructions within newInstructions are
	 * already set as if newInstructions had been inserted.
	 * 
	 * Exception ranges are adjusted for the difference in length between the
	 * old and new instructions. Exception ranges that start or end within the
	 * added instructions throw a CodeCheckException.
	 * 
	...
	/**
	 * Checks the code to make sure each instruction is visited, that there is a
	 * consistent stack depth at each instruction, and determines the maximum
	 * stack depth-- and saves it in the max stack variable.
	 */
	...
	public LineNumberTableAttribute getLineNumberAttribute() {
		return (LineNumberTableAttribute) attributes
				.getAttributeByType(LineNumberTableAttribute.typeString);
	}

	public LocalVariableTableAttribute getLocalVariableTable() {
		return (LocalVariableTableAttribute) attributes
				.getAttributeByType(LocalVariableTableAttribute.typeString);
	}
	...
	public static class CodeException {
		...
		CodeException(DataInputStream classStream) throws IOException {
