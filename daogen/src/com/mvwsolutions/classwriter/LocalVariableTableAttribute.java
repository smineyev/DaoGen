...
public class LocalVariableTableAttribute implements Attribute {
	...
	static class LocalVariableTableEntry {
		...
		LocalVariableTableEntry(DataInputStream stream) throws IOException {
	...
	LocalVariableTableAttribute(DataInputStream stream) throws IOException {
	...
	/**
	 * Return the name and descriptor of a local variable reference, or null if
	 * the appropriate local variable can't be found.
	 * 
	 * @param cw
	 *            ClassWriter containing the method that includes the local
	 *            variable
	 * @param pc
	 *            Program counter (byte offset) of the variable reference in the
	 *            code for the method
	 * @param index
	 *            Index of the local variable in the method's frame
	 */
	public String getLocalVariable(ClassWriter cw, int pc, int index) {
		for (Iterator i = entries.iterator(); i.hasNext();) {
			...
			if (pc >= entry.start_pc && pc < entry.start_pc + entry.length
					&& index == entry.variable_index) {
				return cw.getString(entry.name_index) + " ("
						+ cw.getString(entry.descriptor_index) + ")";
			}
		}
		return null;
	}
